...
function FPDropList( {
	...
	return (
		<DropContainer
			onClick={ ( e ) => dropRemove( e.target ) }
			className="FP_drop_list_"
		>
			<SearchContainer
			...
			>
				...
				<input
					...
					onClick={ ( e: React.MouseEvent< HTMLInputElement > ) =>
						dropClicks( e.currentTarget.value )
					}
					onChange={ ( e: React.ChangeEvent< HTMLInputElement > ) =>
						onChangeHandle( e.target.value, e.target.name )
					}
					placeholder={ label }
				/>
				<DropList
					...
					onClick={ ( e: React.MouseEvent ) =>
						dropSelected( e.target )
					}
					className={ `${ name } ${ 'hide' } ${ 'dropdownlist' } ` }
				>
					{ options }
				</DropList>
			</SearchContainer>
