...
class MasjidJamaah {
	...
	FajrJamah() {
		...
		for ( let i = firstDay.getDate(); i < lastDay.getDate(); i++ ) {
			...
			if (
				...
				this.times.fajr_iqamah =
					hourprayer +
					( 29 + timeMin >= 60 ? 1 : '' ) +
					':' +
					( 29 + timeMin >= 60 ? 0 : 29 + timeMin );
			}
			if (
				+this.times.today >= dayOne &&
				minprayer >= 30 &&
	...
	DuhrJamah() {
		let hourprayer =
			1 * ( this.times.dhuhr + '' ).split( /[^0-9.+-]/ )[ 0 ];
		...
		for ( let i = firstDay.getDate(); i < lastDay.getDate(); i++ ) {
			...
			if (
				...
				this.times.dhuhr_iqamah =
					hourprayer +
					( 28 + timeMin >= 60 ? 1 : '' ) +
					':' +
					( 28 + timeMin >= 60 ? 0 : 28 + timeMin );
			}
			if (
				+this.times.today >= dayOne &&
				minprayer >= 29 &&
	...
	MaghribJamah() {
		let hourprayer =
			1 * ( this.times.maghrib + '' ).split( /[^0-9.+-]/ )[ 0 ];
		let minprayer =
			1 * ( this.times.maghrib + '' ).split( /[^0-9.+-]/ )[ 1 ];
		let date = new Date();
		let firstDay = new Date( date.getFullYear(), date.getMonth(), 1 );
		let lastDay = new Date( date.getFullYear(), date.getMonth() + 1, 0 );
		const dayOne = 1;
	...
	IshaJamah() {
		...
		for ( let i = firstDay.getDate(); i < lastDay.getDate(); i++ ) {
			...
			if (
				...
				this.times.isha_iqamah =
					hourprayer +
					( 25 + timeMin >= 56 ? 1 : '' ) +
					':' +
					( 25 + timeMin >= 60 ? timeMin : 25 + timeMin );
			}
			if (
				+this.times.today >= dayOne &&
				minprayer >= 26 &&
				...
				this.times.isha_iqamah =
					...
					( 40 + timeMin >= 56 ? 1 : '' ) +
					':' +
					( 40 + timeMin >= 60 ? timeMin : 40 + timeMin );
			}
			if (
				+this.times.today >= dayOne &&
				minprayer >= 41 &&
				...
				this.times.isha_iqamah =
					...
					( 59 + timeMin >= 56 ? 1 : '' ) +
					':' +
					( 59 + timeMin >= 56 ? timeMin : 59 + timeMin );
			}
		}
	}
}
